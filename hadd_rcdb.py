...
def main(argv):
        ...
        if MINRUN > 40000 and MINRUN < 60000:
                RCDB_RUNS = db.select_runs("@is_2018production and @status_approved", run_min=MINRUN, run_max=MAXRUN)
        if MINRUN > 70000:
                RCDB_RUNS = db.select_runs("@is_dirc_production and @status_approved", run_min=MINRUN, run_max=MAXRUN)
        ...
        HADD_RUNS_PARA_45_135 = "hadd tree_sum_PARA_135_%d_%d.root" % (MINRUN, MAXRUN)
        HADD_RUNS_PERP_45_135 = "hadd tree_sum_PERP_45_%d_%d.root" % (MINRUN, MAXRUN)
        ...
        for RCDB_RUN in RCDB_RUNS:
                ...
                HIST_FILE = filter(lambda a: FORMATTED_RUN in a, FOUND_HIST_FILES)[0]
